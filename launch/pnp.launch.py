...
def generate_launch_description():
    ...
    run_pnp = LaunchConfiguration("run_pnp")
    ...
    pnp_sender = TimerAction(
        period=6.0,
        actions=[
            Node(
                package="IRB120",
                executable="send_trajectory_pnp",
                output="screen",
                parameters=[
                    {
                        "use_sim_time": use_sim_time,
                        "cube_xyz": [0.35, 0.0, 0.10],
                        "cube_size": 0.05,
                        "approach_above_top": 0.12,
                        "pick_above_top": 0.04,
                        "lift_height": 0.28,
                        "place_xyz": [0.15, -0.25, 0.10],
                        "place_above_top": 0.05,
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "run_pnp",
                default_value="true",
                description="Start pick and place sequence after MoveIt starts.",
            ),
            gazebo_launch,
